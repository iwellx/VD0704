...
class CountsController extends AdminCoreController {
	...
	public function company(){
		...
		if(!empty($get_data['start']) && !empty($get_data['end']))
		{
			...
			//有必要添加$map[]
            $map["FROM_UNIXTIME(create_time, '%Y-%m')"] = array(array('egt',$get_data['start']),array('elt',$end_time));
		...
		{
            $end_time = date('Y-m',strtotime($get_data['end']));
            $map["FROM_UNIXTIME(create_time, '%Y-%m')"] = array('elt',$get_data['end']);
            $map1["FROM_UNIXTIME(create_time, '%Y-%m')"] = array(array('egt',$end_time),array('elt',$get_data['end']));
		...
		{
            $map["FROM_UNIXTIME(create_time, '%Y-%m')"] = array('egt',$get_data['start']);
...
/* 		if(empty($get_data['company_name']))
	...
	public function companyExport(){
		...
		if(!empty($get_data['company_name']))
		{
			$map['unicom_name'] = array('like','%'.$get_data['company_name'].'%');
			$page_search['company_name'] = $get_data['company_name'];
			$search_name = ' , '.'全称中带有'.'"'.$page_search['company_name'].'"';
		}
		if(!empty($get_data['start']) && !empty($get_data['end']))
		{
			$end_time = date('Y-m',strtotime($get_data['end']));
            $map["FROM_UNIXTIME(create_time, '%Y-%m')"] = array(array('egt',$get_data['start']),array('elt',$end_time));
			$map1["FROM_UNIXTIME(create_time, '%Y-%m')"] = array(array('egt',$get_data['start']),array('elt',$end_time));
			$map2["FROM_UNIXTIME(create_time, '%Y-%m')"] = array(array('egt',$get_data['start']),array('elt',$get_data['end']),'and');
			$page_search['start'] = $get_data['start'];
			$page_search['end'] = $get_data['end'];
		...
		$name = $search_time.$search_name.'的公司统计报表';
...
/* 	public function companycount(){
	...
	*/
	public function saleCountsExport()
	{
		...
		if(!empty(trim($get_data['company_name'])))
		{
			...
			$search_name = $get_data['company_name'];
		}
		//查询时间
		if(!empty($get_data['start']) && !empty($get_data['end']))
		{
		...
		if(!empty($list))
		{
			import('Common.Libs.PHPExcel');
			$objPHPExcel = new \PHPExcel();
			$name = $search_time.' , '.$search_name.'销售订单统计表';
	...
	*/
	public function saleCountListExport() {
		...
		for ($p = 1; $p <= $page; $p++) {
			...
			foreach($list as $key=>$value)
			{
				...
				$temp = array(
					...
					'package_cycle' => $value['package_cycle'],
					'order_account' => $value['order_account'],
					'time' => $time,
				);
				foreach ($temp as $i => $v) {
		...
		$name = iconv('utf-8','gb2312','销售统计报表明细.csv');
		
		$str = "客户编号,IMSI,运营商,所属公司,套餐名称,套餐周期,销售价格,销售时间\n";
		$fp = fopen('file.csv','r');
		while ($data = fgetcsv($fp)) {
			$str .= $data[0].',';
			$str .= "\t".$data[1].',';
			$str .= iconv("gbk", "utf-8//IGNORE",$data[2]).',';
			$str .= iconv("gbk", "utf-8//IGNORE",$data[3]).',';
			$str .= iconv("gbk", "utf-8//IGNORE",$data[4]).',';
			$str .= $data[5].',';
			$str .= $data[6].',';
			$str .= $data[7]."\n";
		}
	...
	*/
	public function customCostExport()
	{
		...
		$dbprefix = C('DB_PREFIX');
		$count =$this->Model->table($dbprefix.'custom_handle_log a')->join('LEFT JOIN '.$dbprefix.'company b ON a.company= b.id and a.handle_type=10 and (a.type=10 or a.type=11)')->join('LEFT JOIN '.$dbprefix.'package c ON a.content= c.package_sn')->where($map)->group('a.type,a.content,a.company')->count();
		//print_r($this->Model->getlastsql());exit;
		$page = ceil($count / 10000);
		$fp = fopen('file.csv', 'w');
		header("Content-type:text/html;charset=utf-8");
		$total_custom = $total_money = 0;
		for ($p = 1; $p <= $page; $p++) {
			$list = $this->Model->table($dbprefix.'custom_handle_log a')->join('LEFT JOIN '.$dbprefix.'company b ON a.company= b.id and a.handle_type=10 and (a.type=10 or a.type=11)')->join('LEFT JOIN '.$dbprefix.'package c ON a.content= c.package_sn')->where($map)->group('a.type,a.content,a.company')->order('b.unicom_name asc,a.content asc')->field('a.id,a.company as company_id,b.unicom_name as company_name,c.name as package_name,c.package_sn,c.cycle_value,a.order_account as price,FORMAT((a.order_account/c.cycle_value),2) as package_month_price,COUNT(a.id) as cost_counts,(a.order_account/c.cycle_value)*COUNT(a.id) as total_price,a.type')->limit(($p-1)*10000 . ',10000')->select();
			foreach($list as $key=>$value)
			{
				$temp = array(
					'company_name' => $value['company_name'],
					'package_name' => $value['package_name'],
					'cycle_value' => $value['cycle_value'],
					'price' => sprintf("%2f",$value['price']),
					'package_month_price' =>  sprintf("%2f",$value['package_month_price']),
					'cost_counts' => $value['cost_counts'],
					'total_price' => sprintf("%2f",$value['total_price']),
					'type' => $value['type'],
				);
				$total_custom += $value['cost_counts'];
				$total_money += $value['total_price'];
		...
		$tmp_name = $search_time.'用户月报表';
		$name = iconv('utf-8','gb2312',$tmp_name.'.csv');
		$str = $tmp_name.",1,2,3,4,5,6,7,8,\n";
		$str .= "编号,企业名称,套餐名称,套餐周期,套餐价格,套餐单价,收费用户数,收费总价,收费类型\n";
		$i=1;
		$fp = fopen('file.csv','r');
		while ($data = fgetcsv($fp)) {
			$str .= $i.',';
			$str .= iconv("gbk", "utf-8//IGNORE",$data[0]).',';
			$str .= iconv("gbk", "utf-8//IGNORE",$data[1]).',';
			$str .= $data[2].',';
			$str .= $data[3].',';
			$str .= $data[4].',';
			$str .= $data[5].',';
		...
		$str .= ",,,,,,,".$total_custom.",".sprintf("%2f",$total_money).",\n";
		ftruncate($fp,0);
		rewind($fp);
		fclose($fp);
		$this->export_csv($name,$str);
	...
	*/
	...
	/*
		订单统计明细导出
	*/
	public function customCostListExport()
	{
		$get_data = I('get.');
		$map = array(
			'c.package_sn' => $get_data['package_sn'],
		...
		$count =  $this->Model->join(' a LEFT JOIN '.$dbprefix.'company b ON a.company= b.id and a.handle_type=10 and (a.type=10 or a.type=11)')->join('LEFT JOIN '.$dbprefix.'package c ON a.content= c.package_sn')->join('LEFT JOIN '.$dbprefix.'user_package d ON a.content= d.package_sn and a.custom_no=d.custom_no')->where($map)->count();
		
		$page = ceil($count / 10000);
		$fp = fopen('file.csv', 'w');
		header("Content-type:text/html;charset=utf-8");
		for ($p = 1; $p <= $page; $p++) {
			$list = $this->Model->join(' a LEFT JOIN '.$dbprefix.'company b ON a.company= b.id and a.handle_type=10 and (a.type=10 or a.type=11)')->join('LEFT JOIN '.$dbprefix.'package c ON a.content= c.package_sn')->join('LEFT JOIN '.$dbprefix.'user_package d ON a.content= d.package_sn and a.custom_no=d.custom_no')->join('LEFT JOIN '.$dbprefix.'custom e ON a.custom_no=e.custom_no')->where($map)->order('b.unicom_name asc')->field('a.id,a.custom_no,b.unicom_name as company_name,c.name as package_name,c.package_sn,c.carrieroperator,d.service_start_time,d.service_end_time,e.imsi,e.custom_state')->limit(($p-1)*10000 . ',10000')->select();
			//print_r($this->Model);
			foreach($list as $key=>$value)
			{
				if($value['carrieroperator'] == 10)
				...
				$temp = array(
					...
					'company_name' => $value['company_name'],
					'package_name' => $value['package_name'],
					'time' => date('Y-m',$value['service_start_time']).'-'.date('Y-m',$value['service_end_time']),
		...
		$name = iconv('utf-8','gb2312','用户月报表明细.csv');
		$str = "客户编号,IMSI,运营商,所属公司,套餐名称,服务时间,状态\n";
		$fp = fopen('file.csv','r');
		while ($data = fgetcsv($fp)) {
			$str .= $data[0].',';
			$str .= "\t".$data[1].',';
	...
	public function company_index()
	{
		...
		}elseif(!empty($get_data['start']))
		{
			...
			$map1["FROM_UNIXTIME(create_time, '%Y-%m-%d')"] = array(array('egt',$start_time),array('elt',$end_time),'and');//分期统计时
			//分期统计时间
...
/* 	public function expenseRate()
	...
	public function activityCardCount()
	{
		$get_data = I('get.');
		$dbprefix = C('DB_PREFIX');
		if(!empty(trim($get_data['company_name'])))
		{
			$map1['b.unicom_name'] = array('eq',trim($get_data['company_name']));
			$page_search['company_name'] = $get_data['company_name'];
		}
		
		if(!empty(trim($get_data['sale_start'])))
		{
			$page_search['sale_start'] = $get_data['sale_start'];
		}else{
			$get_data['sale_start'] = date("Y-m",strtotime("-3 month",time()));
		}
		if(!empty(trim($get_data['sale_end'])))
		{
			$page_search['sale_end'] = $get_data['sale_end'];
		}else{
			$get_data['sale_end'] = date("Y-m");
		}
		if(!empty(trim($get_data['activity_start'])))
		{
			$page_search['activity_start'] = $get_data['activity_start'];
		}else{
			$get_data['activity_start'] = date("Y-m",strtotime("-6 month",time()));
		}
		if(!empty(trim($get_data['activity_end'])))
		{
			$page_search['activity_end'] = $get_data['activity_end'];
		}else{
			$get_data['activity_end'] = date("Y-m");
		}
		if($page_search)
		{
			$page_con = http_build_query($page_search);//跳转分页条件检索
		...
		$map1['a.type'] = 13;
		//$map1['a.company'] = array('eq',1);
		header("Content-type:text/html;charset=utf-8");
		$company_list = $this->Model->join(' a LEFT JOIN '.$dbprefix.'company b ON a.company = b.id')->where($map1)->field('a.company,b.unicom_name')->group('a.company')->select();
		
		$get_data['sale_end'] = empty($get_data['sale_end']) ? date('Y-m'):$get_data['sale_end'];
		//$get_data['sale_start'] = empty($get_data['sale_start']) ? date("Y-m",strtotime("-1 year",strtotime(date('Y-m')))):$get_data['sale_start'];
		$get_data['sale_start'] = empty($get_data['sale_start']) ? date("Y-m",strtotime("-5 month",strtotime(date('Y-m')))):$get_data['sale_start'];
		$sale_time_between = $this->getMonthNum($get_data['sale_start'],$get_data['sale_end']);
		$sale_time = '';
		$sale_data = array();
		foreach($company_list as $key=>$value)
		{
			...
			$total_count = 0;
			$temp_data=array();
			for($i=0;$i<=$sale_time_between;$i++)
			{
				$sale_time = date("Y-m",strtotime("+".$i." month",strtotime($get_data['sale_start'])));
				$sale_data[$i]['sale_time'] = date('Y-m',strtotime($sale_time));
				$map1["a.company"] = array('eq',$value['company']);
				$map1["FROM_UNIXTIME(a.create_time, '%Y-%m')"] = array('eq',$sale_time);
				$count = $this->Model->join(' a LEFT JOIN '.$dbprefix.'company b ON a.company = b.id')->where($map1)->count();
				$sale_data[$i]['company'] = $value['company'];
				$sale_data[$i]['company_name'] = $value['unicom_name'];
				$sale_data[$i]['sale_count'] = $count;
				if($count>0)
				{
					$sale_count = 1;
					$total_count +=$count;
				}
				$activity_data = $this->deal_activity_count($sale_data[$i],$get_data);
				$sale_data[$i]['activity_data'] = $activity_data;
				$company_list[$key]['sale_data'] = $sale_data;
			}
			$company_list[$key]['rowspan_count'] = count($company_list[$key]['sale_data'])+1;
			if($sale_count==0)
			{
				...
				{
					foreach($v['activity_data'] as $s)
					{
						$s['activity_time'] && $temp_data[$s['activity_time']]['activity_count'] += $s['activity_count'];
					}
				}
				$sale_data[$i+1]['sale_time'] = '合计';
				$sale_data[$i+1]['sale_count'] = $total_count;
				$sale_data[$i+1]['activity_data'] = $temp_data;
				$company_list[$key]['sale_data'] = $sale_data;
			}
		}
		$this->assign('company_list',array_values($company_list));
		$this->display();
	}
	
	public function deal_activity_count($sale,$get_data = array())
	{
		$dbprefix = C('DB_PREFIX');
		$sale_where = "c.type=13 AND c.company='".$sale['company']."' AND FROM_UNIXTIME(c.create_time, '%Y-%m')='" .$sale['sale_time']. "'";
		$activity_time_between = $this->getMonthNum($get_data['activity_start'],$get_data['activity_end']);
		$activity_time = '';
		$activity_data = array();
		$map['a.type'] = 10;
		for($i=0;$i<=$activity_time_between;$i++)
		{
			$activity_time = date("Y-m",strtotime("+".$i." month",strtotime($get_data['activity_start'])));
			$activity_data[$i]['activity_time'] = date('Y-m',strtotime($activity_time));
			$activity_data[$i]['company'] = $sale['company'];
			$activity_data[$i]['company_name'] = $sale['company_name'];
			if($activity_time<$sale['sale_time'])
			{
				$activity_data[$i]['activity_count'] = 0;
			}else{
				$map["a.company"] = array('eq',$sale['company']);
				$map["FROM_UNIXTIME(a.create_time, '%Y-%m')"] = array('eq',$activity_time);
				$map['_string'] = "EXISTS(SELECT content,custom_no from ".$dbprefix."custom_handle_log c where c.content=a.content and c.custom_no=a.custom_no AND ".$sale_where.")";
				$count = $this->Model->join(' a LEFT JOIN '.$dbprefix.'company b ON a.company = b.id')->where($map)->count();
				$activity_data[$i]['activity_count'] = $count;
			}
		}
	...
	public function activityCardCountExport()
	{
		$get_data = I('get.');
		$dbprefix = C('DB_PREFIX');
		if(!empty(trim($get_data['company_name'])))
		...
		foreach($company_list as $key=>$value)
		{
			...
			for($i=0;$i<=$sale_time_between;$i++)
			{
				...
				if($i==0)
				{
					$str[$key]['content'][$i] .= $sale_data[$i]['company_name'].',';
				}else{
					$str[$key]['content'][$i] .=',';
				}
				$str[$key]['content'][$i] .= $sale_data[$i]['sale_time'].',';
				$sale_data[$i]['sale_count'] = $count;
				$str[$key]['content'][$i] .= $sale_data[$i]['sale_count'].',';
				if($count>0)
				{
					$sale_count = 1;
					$total_count +=$count;
			...
			if($sale_count==0)
			{
				...
				foreach($sale_data as $k=>$v)
				{
					foreach($v['activity_data'] as $s)
					{
						if($k==0)
						{
							$str[$key]['name'] .= $s['activity_time'].',';
						}
						$str[$key]['content'][$k] .= $s['activity_count'].',';
						$s['activity_time'] && $temp_data[$s['activity_time']]['activity_count'] += $s['activity_count'];
					}
				}
				$str[$key]['content'][$i+1] .=',合计,'.$total_count.','.join(',',array_column($temp_data,'activity_count'));
			}
			
		}
		$filename = $page_search['sale_start'].'至'.$page_search['sale_end'].'销售激活统计表';
		$name = iconv('utf-8','gb2312',$filename.'.csv');
		foreach($str as $key=>$value)
		{
			$csv_data .= rtrim($value['name'],',')."\n";
